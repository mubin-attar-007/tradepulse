"""Persisted backtest runs: create/run/store, parity with the engine, endpoints."""
...
def _spec(symbol: str = "BTC/USD") -> StrategySpec:
    return StrategySpec(
        name="bt-sma",
        universe=[symbol],
        timeframe="1m",
...
async def _seed_bars(session: object, symbol: str = "BTC/USD", n: int = 40) -> Instrument:
...
async def test_run_persists_and_matches_engine() -> None:
    async with get_sessionmaker()() as s:
        user = User(email="bt@example.com")
        s.add(user)
        await s.flush()
        inst = await _seed_bars(s)
        spec = _spec()
        strategy, _ = await StrategyService(s, user.id).create(spec)
        await s.commit()

        backtest = await BacktestService(s, user.id).run(strategy.id, BASE, END)
        await s.commit()
        assert backtest.status == "done" and backtest.result is not None

        bars = await md_repo.get_bars(s, inst.id, timeframe="1m", start=BASE, end=END)
        direct = engine.run(spec, bars, ExecutionConfig())
        assert backtest.result["final_equity"] == str(direct.final_equity)
        assert backtest.result["metrics"]["num_trades"] == float(len(direct.trades))
...
async def test_owner_scoping() -> None:
    async with get_sessionmaker()() as s:
        ...
        await s.flush()
        await _seed_bars(s)
        strategy, _ = await StrategyService(s, a.id).create(_spec())
        await s.commit()
        backtest = await BacktestService(s, a.id).run(strategy.id, BASE, END)
        await s.commit()
        from app.modules.backtesting.repository import BacktestRepository

        assert await BacktestRepository(s, b.id).get(backtest.id) is None
...
async def test_backtests_require_auth(client: AsyncClient) -> None:
    assert (await client.get("/backtests")).status_code == 401


async def test_create_backtest_endpoint(client: AsyncClient) -> None:
    async with get_sessionmaker()() as s:
        await _seed_bars(s)
    await _login(client)
    created = await client.post("/strategies", json=_spec().model_dump(mode="json"))
    strategy_id = created.json()["strategy"]["id"]
    resp = await client.post(
        "/backtests",
        json={"strategy_id": strategy_id, "start": BASE.isoformat(), "end": END.isoformat()},
    )
    assert resp.status_code == 201, resp.text
    body = resp.json()
    assert body["status"] == "done"
    assert body["result"]["num_trades"] >= 0
